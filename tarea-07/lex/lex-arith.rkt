...
(define reg-var
  (reg-conc "v" "a" "r" " " (reg-repeat 1 +inf.0 (char-set "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"))))
...
(define lex-arith
  (make-lexer
   ...
   (lex-rule reg-then lex-func)
   (lex-rule reg-else lex-func)
   ;; delimiters
...
(module+ test
  ...
  (run-tests
   (test-suite
    ...
    (check-equal? (lex "if #t then 2 else #f")
                  '((func if)
                    (bool #t)
                    (func then)
                    (nat 2)
    ...
    (check-equal? (lex "let (var x 8) (2 + var x)")
                  '((func let)
                    (opar #f)
                    (var #f)
                    (nat 8)
